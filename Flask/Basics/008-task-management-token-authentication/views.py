    ...
    def wrapper(*args, **kwargs):
        ...
        if username and password:
            ...
            try:
                ...
                auth.username = username  # store username in the `auth` object for later use
...
def mark_completed(task_id):
    ...
    if task["username"] != request.authorization.username:
        # say to a user that they are not allowed to mark this task as completed and provide 403 status code
...
def delete(task_id):
    ...
    if not task:
        ...
        return make_response({"message": "Task not found"}, 404)
    if task["username"] != request.authorization.username:
        # say to a user that they are not allowed to delete this task and provide 403 status code
        return make_response({"message": "Access denied"}, 403)
    
    task_storage.pop(task_id)  # delete the task from the storage
